...
[assembly: Chenyuan.Assemblies.PreApplicationStartMethod(typeof(PluginManager), "Initialize",true)]
...
namespace Chenyuan.Plugins
{
	...
	public class PluginManager
	{
		...
		{
			if (AppVersion.DefaultAppVersion.FullVersion == descriptor.MinAppVersion)
			{
				return true;
			}
			...
			foreach (var version in AppVersion.DefaultAppVersion.BreakingChangesHistory)
			{
		...
		private static Assembly PerformFileDeploy(FileInfo plug)
		{
			if (plug.Directory == null)
				throw new InvalidOperationException("The plugin directory for the " + plug.Name +
													" file exists in a folder outside of the allowed Lifenxiang folder hierarchy");
			...
			//if (CommonHelper.Default.GetTrustLevel() != AspNetHostingPermissionLevel.Unrestricted)
			//{
